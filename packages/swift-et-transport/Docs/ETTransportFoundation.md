...
SSH bootstrap argv is constructed as `ssh -T [-p port] host etterminal --protocol 6`; no post-host `--` is inserted because OpenSSH passes post-host argv as the remote command. The typed bootstrap builder emits `id/passkey_<TERM>\n`, validates and bounds TERM, and never places the id, passkey, or TERM in argv or error strings.
